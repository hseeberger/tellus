...
Messaging benchmarks comparing [ferrier](../ferrier) against two actively maintained Rust actor
...
ferrier's own build and out of the per-pull-request CI.
...
`target/criterion` tree used by ferrier's own regression benchmarks, so the two never mix.

Runs published by CI are collected on the
[comparison dashboard](https://hseeberger.github.io/ferrier/comparison/), one directory per version
tag and per manual run.

## Benchmarks

Three shapes, modeled on [`ferrier/benches/messaging.rs`](../ferrier/benches/messaging.rs):
...
stable across machines and published results remain comparable. Also unlike the ferrier bench,
`fan_out` has the bench thread rather than a root actor distribute the messages.

## Why these two competitors

Both are actively released and, decisively, both run on a plain Tokio runtime like ferrier, which
...
2. **Unbounded mailboxes everywhere.** ferrier defaults to unbounded, ractor is unbounded, and kameo
...
3. **Non-blocking fire-and-forget sends only.** ferrier `ActorRef::tell`, kameo
...
4. **Identical timing boundaries.** Every framework goes through the same `measure` helper: spawning
   ...
   `ping_pong` that includes both actors of a pair: ferrier tears the ponger down as a child of the
...
Both competitors ship per-message instrumentation enabled by default, which ferrier has no equivalent
of (ferrier depends on `tracing` too, but emits nothing per message). Benchmarking them as-shipped
would charge them for an observability feature while measuring ferrier without one, so both are built
...
for a comparison published by ferrier's own maintainer. Anyone reproducing the out-of-the-box
...
1. **ferrier's `receive` is synchronous; kameo's and ractor's handlers are `async fn`.** ferrier
   ...
   ferrier on exactly these microbenchmarks.
2. **ferrier's mailbox is statically typed; the others erase message types.** kameo and ractor box
   messages to support their richer messaging APIs, which costs an allocation and a dynamic dispatch
   per message that ferrier does not pay.
3. **These are messaging microbenchmarks only.** They say nothing about supervision, distribution,
   ...
   feature-rich frameworks; ferrier is under active development and does far less.
...
5. **Written and run by ferrier's maintainer.** The methodology and every line of the benchmark are in
