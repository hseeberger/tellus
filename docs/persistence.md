...
The [`EventSourced`](../tellus/src/persistence/event_sourced.rs) trait mirrors the shape of
[`Actor`](../tellus/src/actor.rs), split along the event-sourcing seams:
...
[`Persistence`](../tellus/src/persistence.rs) wiring, an event store plus optional snapshot store
...
full replay (see schema evolution below). A failure of `recovered`, or a failure or panic of a
store itself while loading the snapshot or reading events, is an ordinary startup failure, e.g. a
struggling
...
self-describingly by a [`Codec`](../tellus/src/persistence/codec.rs), CBOR by default. Old
