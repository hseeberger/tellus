...
- typed, synchronous message handling via the [`Actor`](../tellus/src/actor.rs) trait: `receive`
...
- an actor tree: an [`ActorSystem`](../tellus/src/actor_system.rs) spawns the root actor and any
  actor spawns children via [`ActorContext::spawn`](../tellus/src/actor_context.rs); stopping an
  actor stops its whole subtree, children first,
- fire-and-forget, at-most-once messaging via [`ActorRef::tell`](../tellus/src/actor_ref.rs),
  with undeliverable messages dropped and logged as dead letters,
- request-response on top of that delivery: [`ActorRef::ask`](../tellus/src/actor_ref.rs) awaits a
  reply from outside the actor tree and
  [`ActorContext::reply_to`](../tellus/src/actor_context.rs) lets actors reply to each other
...
- supervision: an actor failing with an error or a panic is stopped or restarted according to
  its [`SupervisionStrategy`](../tellus/src/actor_config.rs),
...
[`ActorConfig`](../tellus/src/actor_config.rs), passed via `spawn_with_config` or
...
carries a [`ReplyTo`](../tellus/src/ask.rs), a single-shot destination for the reply: the
...
[`ActorRef::ask`](../tellus/src/actor_ref.rs) is the boundary API, for code outside of any actor:
...
[`ActorContext::reply_to`](../tellus/src/actor_context.rs) is the actor side: it creates a
...
[`mailbox.rs`](../tellus/src/mailbox.rs). A mailbox is a FIFO channel of `Incoming<M>` split into
...
([`backoff.rs`](../tellus/src/backoff.rs)), capped at `backoff.max()`; once a streak exceeds
...
A watcher (in [`mailbox.rs`](../tellus/src/mailbox.rs)) is essentially a sending handle into the
...
[`actor_context.rs`](../tellus/src/actor_context.rs)), so the panic is logged and termination
