    ...
    Self: Send + 'static,
{
    ...
    ///
    /// # Errors
    /// Implementations fail if the seeds cannot be resolved right now, e.g. the resolver or the
...
/// detection and downing from then on. [BootstrapError::Downed] means the caller should exit so
/// a restart mints the fresh incarnation. A resolve failure is logged and retried, like a seed
/// which is not a member yet, and restarts the settle window, since nothing was observed during
/// it. A settled view holding fewer than `min_peers` addresses is logged as well, since it blocks
/// every decision below and would otherwise be silent. There is no internal timeout, so wrap the
/// call in [timeout](tokio::time::timeout) to bound bootstrap. Nodes deciding on disjoint
/// discovery views can still form separate clusters, which never merge on their own; the settle
/// window and the `min_peers` floor are the guard, so size `min_peers` to the deployment when
/// possible.
///
/// # Errors
/// Fails if the configuration is invalid, if the endpoint is not started, or if this node has
/// been downed, which can only happen when rejoining after a down.
