...
impl QuicTransport {
    ...
    ///
    /// # Errors
    /// Fails if `server_name` is neither a valid DNS name nor an IP address, so a name no
    /// certificate could be issued for is refused here rather than at the first connection
    /// attempt, or if the UDP socket cannot be bound to `bind_addr`.
    pub fn new(
    ...
    ///
    /// # Errors
    /// Fails if a PEM file cannot be read, holds no item or holds something other than what it
    /// must, if a root certificate cannot be added, or if [QuicTransport::mutual_tls] fails on
    /// what was read.
    ...
    ///
    /// # Errors
    /// Fails if the client certificate verifier cannot be built from `roots`, if `cert_chain` and
    /// `key` do not make a valid TLS configuration, if that configuration cannot back QUIC, or if
    /// [QuicTransport::new] fails.
    pub fn mutual_tls(
        bind_addr: SocketAddr,
    ...
    ///
    /// # Errors
    /// Fails if the self signed certificate cannot be generated, if it does not make a valid TLS
    /// configuration, if that configuration cannot back QUIC, or if [QuicTransport::new] fails.
    #[cfg(feature = "cluster-dev")]
    ...
    ///
    /// # Errors
    /// Fails if the bound socket cannot report its address.
