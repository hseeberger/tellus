...
mod tests {
    ...
    /// Registration snapshots ride the control stream, in order with gossip.
    #[test]
    fn registrations_ride_the_control_stream() {
        assert_eq!(
            Frame::Registrations {
                version: 0,
                chunk: 0,
    ...
    /// The registrations variant was appended at discriminant 12, pinned like the others.
    #[test]
    fn registrations_match_their_pinned_wire_bytes() {
        let bytes = [12, 0, 0, 1, 0];

        let frame = Frame::from_bytes(&bytes).expect("frame decodes");
        assert!(matches!(
            frame,
            Frame::Registrations { version: 0, chunk: 0, chunks: 1, ref entries } if entries.is_empty()
        ));
        assert_eq!(
            Frame::Registrations {
                version: 0,
