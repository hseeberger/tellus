...
//! lists them, [cluster_state] holds the current view and [changes] streams every version of it,
//! and [down] removes one by operator decision. A node shutting down cleanly announces
...
//! has to be configured. The [receptionist] is the directory over the same registrations:
//! [receptionist::lookup] answers every actor under a key on every Up member without a round
//! trip, [receptionist::subscribe] watches that set, and [receptionist::settled] tells a complete
//! answer from one still arriving. [pubsub] is that directory used for fan-out:
//! [pubsub::subscribe] registers a local actor under a [pubsub::Topic] and [pubsub::publish] tells
//! every subscriber the receptionist resolves. The seed addresses themselves can be discovered:
//! [bootstrap](fn@bootstrap) resolves them through a [SeedDiscovery], waits for the view to settle,
//! and joins through it. The provided discoveries are [FixedSeeds] here, DNS in the
//! `tellus-bootstrap-dns` crate and Kubernetes pods in the `tellus-bootstrap-k8s` crate. When no
//! resolved address is a member of anything, bootstrap forms a cluster as its [FormationProvider]
//! decides, by default [Majority]: at the lowest address of a majority. Requiring a majority keeps
//! a partitioned node from coming back as a cluster of its own. Bootstrap runs once; from then on
//! membership owns failure detection and downing. Alternatively [serialize_ref] turns a local
//! reference into bytes to be shared via configuration, command line or any other channel, and
//! [deserialize_ref] resolves them on another node, refusing bytes serialized for another message
//! type as [RefError::TypeMismatch]. Any further reference travels inside messages.
//!
//! # Guarantees
//!
...
pub use crate::cluster::{
    ...
    endpoint::{
        EndpointConfig, FormError, InvalidEndpointConfig, Lifecycle, StartError, form,
        start_endpoint,
    },
    ...
    node::Incarnation,
    state::{
        Change, Changes, ClusterState, ClusterStateError, Transition, changes, cluster_state,
        lifecycle,
    },
