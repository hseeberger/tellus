...
//!
//! Run it via `just run-examples-event-sourced-counter`, which starts the `postgres` service of
//! the repository's docker-compose.yaml via `docker compose up -d --wait postgres`. The database
//! is addressed via `DATABASE_URL`, defaulting to that service; `docker compose down -v postgres`
//! resets the count.
